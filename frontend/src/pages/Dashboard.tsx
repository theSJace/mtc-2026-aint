...
export function Dashboard() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === "NRIC_IS_PRIMARY")
        setFormError(t.dashboard.nricPrimaryAccount);
  ...
  const statusLabel =
    t.dashboard.statuses[
      user.membership_status as keyof typeof t.dashboard.statuses
    ];
  const isActive = user.membership_status !== "NOT_REGISTERED";
  const monthlyFee =
    user.membership_status === "PINTAR_PLUS"
      ? "$20"
      : user.membership_status === "PINTAR"
        ? "$5"
        : "—";
  ...
  const tabs: { id: Tab; icon: React.ReactNode; label: string }[] = [
    ...
    },
    {
      id: "dependants",
      icon: <Users size={16} />,
      label: t.dashboard.tabs.dependants,
    },
    {
      id: "payments",
    ...
    <div className="min-h-screen bg-cream">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-cream/95 backdrop-blur-xl border-b border-gold/20 px-5 py-3 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-green-deep flex items-center justify-center text-white text-sm">
            🕌
          </div>
          <strong className="font-playfair text-sm text-green-deep">
            Skim Pintar
          </strong>
        </Link>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-text-mid hover:text-green-deep transition-colors"
          >
            <LogOut size={14} />
      ...
      <div className="pt-[60px] flex min-h-screen">
        ...
        <aside className="hidden md:flex flex-col w-56 border-r border-gold/10 bg-white pt-8 pb-6 px-4 fixed top-[60px] bottom-0 left-0">
          ...
          <div className="mt-auto">
            <button
              onClick={handleSignOut}
              className="flex items-center gap-2 px-3 py-2.5 text-sm text-text-light hover:text-red-600 w-full rounded-lg hover:bg-red-50 transition-colors"
            >
              <LogOut size={15} />
              {t.dashboard.signOut}
            </button>
        ...
        <main className="flex-1 md:ml-56 px-4 md:px-8 py-6 pb-24 max-w-4xl">
          ...
          {tab === "overview" && (
            <div className="flex flex-col gap-6">
              <div>
                ...
                <h1 className="font-playfair text-2xl text-green-deep">
                  {user.full_name}
                </h1>
              ...
              <div
                className={cn(
                  ...
                  isActive
                    ...
                    : "bg-white border-2 border-dashed border-green-pale",
                )}
              >
                {isActive && (
                  <>
                    ...
                    <div>
                      <div
                        className={cn(
                          ...
                          isActive ? "text-white/60" : "text-text-light",
                        )}
                      >
                ...
                {[
                  {
                    ...
                    value: dependants.length.toString(),
                    icon: "👨‍👩‍👧",
                  },
                  {
                    label: "Payment Method",
                    value: payments.some((p) => p.status === "COMPLETED")
                      ? "Active"
                      : "Pending",
                    icon: "💳",
                  },
                  {
                    label: "Coverage",
                    value: isActive ? "Full" : "None",
                    icon: "🛡️",
                  },
                ].map((s) => (
                  <div
                    key={s.label}
                    className="bg-white rounded-xl border border-green-pale p-4 text-center"
                  >
                    <div className="text-2xl mb-1.5">{s.icon}</div>
                    <div className="font-semibold text-green-deep text-sm">
                      {s.value}
                    </div>
              ...
              <div className="bg-white rounded-2xl border border-green-pale p-5">
                <h3 className="text-sm font-semibold text-green-deep mb-4">
                  Quick Actions
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    {
                      label: isActive ? "Make Payment" : "Activate Now",
                      icon: "💰",
                      action: () =>
                        navigate(isActive ? "/payment-setup" : "/select-tier"),
                    },
                    {
                      label: "View History",
                      icon: "📋",
                    ...
                    >
                      ...
                      <span className="text-[11px] text-text-mid font-medium leading-tight">
                        {a.label}
                      </span>
                    </button>
                  ))}
          ...
          {tab === "dependants" && (
            <div className="flex flex-col gap-5">
              ...
              >
                <div>
                  <h2 className="font-playfair text-2xl text-green-deep">
                    {t.dashboard.dependants}
                  </h2>
                  <p className="text-xs text-text-light mt-0.5">
                    Family members under your Skim Pintar coverage
                  </p>
                </div>
                {dependants.length > 0 && (
                  <Button
              ...
              {isLoadingDeps ? (
                <div className="text-center py-12 text-text-light text-sm animate-pulse">
                  {t.common.loading}
                </div>
              ) : dependants.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-green-pale p-12 text-center">
                  ...
                  <p className="text-text-mid text-sm mb-5">
                    {t.dashboard.noDependants}
                  </p>
                  <Button
                    onClick={openAdd}
                    variant="secondary"
                    className="border border-green-deep/20 rounded-xl gap-2"
                  >
                    <Plus size={14} />
                ...
                <div className="flex flex-col gap-3">
                  {dependants.map((dep) => (
                    <div
                      key={dep.id}
                      className="bg-white rounded-2xl border border-green-pale px-5 py-4 flex items-start justify-between gap-4"
                    >
                      <div className="flex gap-4 items-start min-w-0">
                        <div className="w-10 h-10 rounded-full bg-green-pale flex items-center justify-center text-lg flex-shrink-0">
                          {dep.relationship === "Children"
                            ? "👧"
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-green-deep text-sm truncate">
                            {dep.full_name}
                          </div>
                          <div className="text-xs text-text-light mt-0.5">
                            {t.signUp.relationshipLabels[
                              dep.relationship as keyof typeof t.signUp.relationshipLabels
                            ] ?? dep.relationship}{" "}
                            · Born{" "}
                          ...
                          {dep.nric && (
                            <div className="text-xs text-text-light font-mono mt-0.5">
                              {dep.nric}
                            </div>
                          )}
                          <div className="text-xs text-text-light mt-0.5">
                            {dep.same_address
                              ? t.dashboard.sameAddressLabel
                              : dep.address}
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-2 flex-shrink-0">
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-green-pale text-green-deep flex items-center justify-center hover:bg-green-soft hover:text-white transition-colors"
                        >
                          <Pencil size={13} />
                        </button>
                        <button
            ...
            <div className="flex flex-col gap-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-text-light mt-0.5">
                    All your Skim Pintar transactions
                  </p>
                </div>
                {isActive && (
                  <Button
                ...
                <div className="bg-amber-50 rounded-2xl border border-amber-200 px-5 py-4 flex gap-3 items-center">
                  ...
                  <div>
                    <div className="text-sm font-medium text-amber-900">
                      {t.dashboard.noActiveMembership}
                    </div>
                    <div className="text-xs text-amber-700 mt-0.5">
                      <Link to="/select-tier" className="underline">
                        {t.dashboard.selectTier}
                      </Link>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-cream border-b border-green-pale">
                        <th className="px-5 py-3 text-left text-xs font-semibold text-text-light uppercase tracking-wider">
                          {t.payment.period}
                        </th>
                        <th className="px-5 py-3 text-left text-xs font-semibold text-text-light uppercase tracking-wider">
                    ...
                    <tbody className="divide-y divide-green-pale/50">
                      {payments.map((p) => {
                        ...
                        > = {
                          COMPLETED: {
                            color: "bg-green-pale text-green-deep",
                            label: t.dashboard.statuses2.COMPLETED,
                          },
                          PENDING: {
                            color: "bg-amber-100 text-amber-700",
                            label: t.dashboard.statuses2.PENDING,
                          },
                          FAILED: {
                            color: "bg-red-100 text-red-700",
                            label: t.dashboard.statuses2.FAILED,
                          },
                        };
                        ...
                        return (
                          ...
                          >
                            <td className="px-5 py-3.5 text-text-mid">
                              {periodLabel}
                            </td>
                            <td className="px-5 py-3.5 text-text-mid">
                            ...
                            <td className="px-5 py-3.5 font-mono text-xs text-text-dark">
                              {p.payment_type}
                            </td>
                            <td className="px-5 py-3.5 font-semibold text-green-deep">
                              ${p.amount.toFixed(2)}
                            </td>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="bg-white rounded-2xl border border-green-pale p-6 flex flex-col gap-4">
                <div className="flex items-center gap-4 pb-4 border-b border-green-pale/50">
                  ...
                  <div>
                    <div className="font-semibold text-green-deep text-lg">
                      {user.full_name}
                    </div>
                    <div className="text-xs text-text-light mt-0.5">
                      {user.email}
                    ...
                    <span
                      className={cn(
                        "mt-1.5 inline-block text-xs font-bold px-2.5 py-0.5 rounded-full",
                ...
                <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4">
                  {[
                    ...
                    },
                    {
                      ...
                      value:
                        user.address +
                        (user.postal_code ? ` S(${user.postal_code})` : ""),
                    },
                    {
                    ...
                    <div key={label}>
                      <div className="text-[11px] text-text-light uppercase tracking-wide mb-1">
                        {label}
                      </div>
      ...
      {showForm && (
        ...
        >
          <Card
            className="w-full max-w-[480px] rounded-2xl border border-gold/20 shadow-2xl bg-cream max-h-[90vh] overflow-y-auto"
          ...
          >
            <CardHeader className="pb-2 sticky top-0 bg-cream z-10 border-b border-green-pale/50">
              <CardTitle className="text-lg font-playfair text-green-deep">
                {editingId
                  ? t.dashboard.edit + t.dashboard.editFamilyMember
                  : t.dashboard.addDependant}
              </CardTitle>
            </CardHeader>
              ...
              <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
                  ...
                  >
                    ...
                    {RELATIONSHIP_OPTIONS.map((opt) => (
                      <option key={opt} value={opt}>
                        {t.signUp.relationshipLabels[opt]}
                      </option>
                    ))}
                  </select>
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <Input
                    value={form.nric}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, nric: e.target.value }))
                    }
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, sameAddress: e.target.checked }))
                    }
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button
                    ...
                    variant="secondary"
                    onClick={() => setShowForm(false)}
                    className="flex-1 rounded-xl h-11 border border-green-deep/20"
                  >
                    {t.common.cancel}
                  </Button>
                </div>
