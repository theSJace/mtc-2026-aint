        ...
        <div className="grid md:grid-cols-2 gap-14 items-center">
          ...
          <div>
            ...
            <h1 className="font-playfair text-4xl md:text-5xl lg:text-6xl leading-tight text-green-deep mb-5">
              {t.hero.title1}<br />
              {t.hero.title2}<br />
              {t.hero.title3} <em className="text-gold not-italic">{t.hero.titleAccent}</em>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-9">
              <Link to="/sign-in"><Button>{t.hero.loginBtn}</Button></Link>
              <Button variant="secondary">{t.hero.helpBtn}</Button>
            </div>

            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-1.5 text-xs text-text-light">
                <div className="w-6.5 h-6.5 bg-green-pale rounded-full flex items-center justify-center text-sm">🔒</div>
                {t.hero.pdpa}
              </div>
              <div className="flex items-center gap-1.5 text-xs text-text-light">
                <div className="w-6.5 h-6.5 bg-green-pale rounded-full flex items-center justify-center text-sm">✅</div>
          ...
          <div className="hidden md:flex justify-center items-center">
            <div className="relative w-full max-w-[370px] h-[410px]">
              ...
              <div className="absolute top-0 left-0 w-[92%] h-[96%] bg-white rounded-3xl shadow-lg p-6 flex flex-col gap-3.5">
                <div className="w-13 h-13 bg-green-deep rounded-xl flex items-center justify-center text-2xl">🕌</div>
                <div>
                  <div className="font-playfair text-lg font-semibold text-green-deep">Skim Pintar</div>
                  <div className="text-[11px] text-text-light">{t.hero.cardSubtitle}</div>
                </div>
                <div className="bg-green-pale rounded-xl p-3 flex justify-between items-center">
                  <div>
                    <div className="text-[10px] text-text-light">{t.hero.communityMembers}</div>
                ...
                <div className="flex items-start gap-2.5 bg-cream rounded-xl p-3 mt-auto">
                  ...
                  <div className="text-[11px] text-text-mid leading-relaxed">
                    <strong className="block text-green-deep text-xs mb-0.5">{t.hero.sadaqahTitle}</strong>
                    {t.hero.sadaqahDesc}
                  </div>
                </div>
              </div>
              <div className="absolute right-[-8px] top-11 bg-white rounded-xl px-3.5 py-2.5 shadow-lg text-[11px] font-semibold text-green-deep animate-float">
                {t.hero.urusanJenazahBadge}
                <span className="block font-normal text-text-light text-[10px] mt-0.5">{t.hero.coveredForFamily}</span>
              </div>
              <div className="absolute left-[-8px] bottom-16 bg-white rounded-xl px-3.5 py-2.5 shadow-lg text-[11px] font-semibold text-brown-warm animate-float-delayed">
                {t.hero.no30DayWait}
                <span className="block font-normal text-text-light text-[10px] mt-0.5">{t.hero.payNowInstant}</span>
              </div>
            </div>
          </div>
        </div>
